...
export function TaskDetail({
  ...
  // Update isEditing state when initialEditMode changes
  useEffect(() => {
    setIsEditing(initialEditMode);

    // If we're opening in edit mode, make sure edit values are initialized
  ...
  useEffect(() => {
    if (initialTask) {
      ...
      setEditValues({
        ...
        assigneeId: initialTask.assigneeId || "unassigned",
      });
      return;
    }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (onTaskUpdate) {
        await onTaskUpdate(task.id, editValues);
      }
  ...
  const toggleEditMode = () => {
    if (isEditing) {
      // If we're exiting edit mode without saving, reset the edit values
      setEditValues({
        title: task?.title || "",
        status: task?.status || "",
        priority: task?.priority || "",
